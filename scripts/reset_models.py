"""
Reset models - Delete all existing models and create new ones
"""
import sys
...
def reset_models():
    """Delete all existing models and portfolios, then create new ones"""
    db = SessionLocal()

    try:
        # Delete all existing data
        logger.info("Deleting existing data...")
        db.query(Reasoning).delete()
        ...
        # New models - User's preferred models
        demo_models = [
            {
        ...
        logger.info(f"Creating {len(demo_models)} new models...")
        ...
        db.commit()
        logger.info(f"Successfully created {len(demo_models)} new models!")
        logger.info("\nModels created:")
        for model_data in demo_models:
            logger.info(f"  - {model_data['name']}: {model_data['api_identifier']}")

    except Exception as e:
        logger.error(f"Failed to reset models: {e}")
        db.rollback()
        raise
    finally:
        db.close()
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("  WARNING: This will DELETE all existing trading data!")
    print("="*60)
